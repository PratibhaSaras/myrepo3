...
public class RegisterPage {
	...
	@FindBy(xpath="//a[normalize-space()= 'Signup / Login']") WebElement loginBtn;
	...
	@FindBy(xpath="//b[normalize-space()='Enter Account Information']") WebElement SignUpPageText;

	@FindBy(id="uniform-id_gender1") WebElement male;

	@FindBy(id="uniform-id_gender2") WebElement female;

	@FindBy(xpath="//input[@data-qa='password']") WebElement password;

	@FindBy(xpath="//select[@id='days']") WebElement dobDay;

	@FindBy(xpath="//select[@id='months']") WebElement dobMonth;

	@FindBy(xpath="//select[@id='years']") WebElement dobYear;

	@FindBy(xpath="//input[@id='newsletter']") WebElement newsletter;

	@FindBy(xpath="//input[@id='optin']") WebElement opt_in;
	...
	@FindBy(xpath="//button[@data-qa='create-account']") WebElement createAcc_btn;

	@FindBy(xpath="//b[normalize-space()='Account Created!']") WebElement AccCreation_text;
	
	@FindBy(linkText="Continue") WebElement continue_btn;
	
	@FindBy(xpath="//a[text()=' Logged in as ']") WebElement login_text;
	
	@FindBy(xpath="//b") WebElement user;
	
	@FindBy(xpath="//a[text()=' Delete Account']") WebElement del_btn;
	
	@FindBy(xpath="//b[text()='Account Deleted!']") WebElement del_confirmation_text;
	

	public void validateHomePageIsReached()
	{	
	...
	public void verifyLoginPageVisible()
	{
		...
		Assert.assertEquals(signUpText.getText(), "New User Signup!");
	}
	...
	public void verifySignUpFormVisible() {
		Assert.assertEquals(SignUpPageText.getText(), "ENTER ACCOUNT INFORMATION");
	}
...
/*	private boolean checkSorting(List<WebElement> days) {
		...
		//convert List of WebElement to List of String using java 8
		List<String> daysJava8 = days.stream().map(obj->obj.getText()).skip(1).toList();

		//convert List of WebElement to List of String using forEach
		List<String> daysSimple = new ArrayList<>();
		for(WebElement we : days) {
			daysSimple.add(we.getText());
		}
		daysSimple.remove(0);
		
		daysJava8.stream().forEach(System.out::print);

		//create a set from the days list to remove duplicates
		Set<String> temp= new TreeSet<String>(daysSimple);

		//check unique
		if(daysSimple.size()!=temp.size()) {
			System.out.println("check unique");
			return false;
		}

		//check sorting using java 8
		List<String> daysSortedJava8 = daysJava8.stream().sorted().toList();
		daysSortedJava8.stream().forEach(System.out::print);
		if(!(daysSortedJava8.equals(daysJava8))) {
			System.out.println("check sorting using java 8");
			//return false;
		}

		//check sorting using for loop
		for(int i=0; i<days.size()-1;i++) {
			System.out.println(daysJava8.get(i));
			System.out.println(daysJava8.get(i+1));
			System.out.println(daysJava8.get(i).compareTo(daysJava8.get(i+1)));
			if(daysJava8.get(i).compareTo(daysJava8.get(i+1))>1) {
				System.out.println("check sorting using for loop");
				return false;
			}
		}
	...
	public void validateAccCreation() {
		System.out.println(AccCreation_text);
		Assert.assertEquals(AccCreation_text.getText(), "ACCOUNT CREATED!");
	}
	...
	public void validateLogin() {
		String s= login_text.getText();
		Assert.assertEquals(s, "Logged in as "+user.getText());
	}
